	    ...
	    class="form form-horizontal well" >
			...
			<c:forEach items="${daysOfWeek}" var="day" varStatus="status">
				<div class="control-group">
					...
					<c:choose>
					   <c:when test="${weekEntry.getIsReadOnly()}"><input name="expenses" class="input-block-level" id="${status.index}" type="text" value="${entries[status.index]}" disabled/></c:when>
					   <c:otherwise><input name="expenses" class="input-block-level" id="${status.index}" type="text" value="${entries[status.index]}" enabled/></c:otherwise>
					</c:choose>
				</div>
			</c:forEach>
			<c:choose>
