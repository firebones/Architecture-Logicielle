...
public class WeekEntryRepositoryImpl implements WeekEntryRepository {
	...
	@Override
	public WeekEntry getWeekEntryByEmailAndWeekAndYear(String email,
			Integer weekNumber, Integer yearNumber) {

		ArrayList<String> listElement = new ArrayList<String>();

		listElement = xmlWeekPersistance.getWeekEntryByEmailAndWeek(email,
	...
	private WeekEntry createWeekEntry(ArrayList<String> tabweek) {
		...
		case "Approved":
			state = StateWeekEntry.APPROVED;
			break;
		case "Refused":
			state = StateWeekEntry.REFUSED;
			break;
		case "Submitted":
			state = StateWeekEntry.SUBMITTED;
			break;
		case "inProgress":
		...
		periodPay.setInformation(tabweek.get(0),
				Integer.parseInt(tabweek.get(1)),
				Integer.parseInt(tabweek.get(2)), state);
	...
	private ArrayList<String> getweekEntryString(WeekEntry periodPay) {
		...
		int j = 5;
		if (periodPay instanceof WeekEntry
				&& periodPay.getKilometersEntries().size() > 0) {
